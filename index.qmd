...
human PET and fMRI localizers that isolated a ventral-occipital colour-responsive
region (hV4 / V4α) [@lueck1989; @mckeefry1997; @bartels2000], and to the clinical
...
- **V4** is chromatically biased (color > luminance, like higher visual) but weak
  on raw pixel statistics — consistent with its role in *perceptual/constant*
  color (a stable surface color across changing illumination) rather than reading
  raw wavelength. The raw-pixel targets used here do not play to that specialty.
...
not separate face/place/body/word regions (which requires raw-NSD ROI masks — the
accompanying repository supports that path).
