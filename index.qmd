...
ROI differences are tested across subjects (`stats`), not read off the SEM bars:
each contrast uses an exact paired **sign-flip permutation test** (all 2^8 sign
combinations), a 95% **bootstrap confidence interval**, and **Benjamini-Hochberg
FDR** correction across the family of contrasts. With n = 8 the permutation
*p* has a floor of 2/2⁸ ≈ 0.008 — so *p* = 0.008 means the observed difference is
more extreme than all other sign combinations, i.e. as strong as eight subjects
can show, not a marginal result.
