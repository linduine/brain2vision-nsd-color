...
The bar charts show each ROI's decoding as mean ± SEM across the eight subjects,
which gives a visual feel for which differences look dependable. But eyeballing
whether two error bars overlap isn't really a test, so we wanted to check the ROI
differences properly — and, most of all, to see whether the colour/luminance
crossover is a genuine interaction rather than two effects that happen to point
opposite ways.

To do that we compare regions *within* each subject (the same eight people see
both), using an exact paired sign-flip permutation test (it assumes nothing about
the distribution, which suits a small sample), a 95% bootstrap confidence interval
for the size of each difference, and a Benjamini-Hochberg FDR correction because
we're looking at a family of contrasts. The implementation is the
[`stats`](https://github.com/linduine/brain2vision-nsd-color/blob/main/brain2vision/stats.py)
module. One number worth reading carefully: with only eight subjects the
permutation *p* can't drop below 2/2⁸ ≈ 0.008, so a *p* of 0.008 isn't "barely
significant" — it means the difference is more extreme than every other way the
signs could have fallen, i.e. as strong as eight subjects can show.

The eight contrasts are in @tbl-stats and @fig-stats. Reassuringly, every ROI
comparison survives FDR correction except early visual vs V4 on colour, which
comes out a genuine tie — and the interaction we cared about most (higher visual
being more colour-biased than early visual) is the largest and most reliable
effect of the set.

| contrast | Δ R² | *p* | *q* (FDR) | 95% CI |
|---|---:|---:|---:|:---:|
| colour: concept − early | +0.012 | 0.008 | 0.01 | [0.007, 0.018] |
| colour: concept − V4 | +0.016 | 0.008 | 0.01 | [0.012, 0.020] |
| colour: early − V4 | +0.004 | 0.141 | 0.14 | [−0.001, 0.007] |
...
colour R² − luminance R² (the dissociation interaction). {#tbl-stats}

![ROI contrasts (n = 8). Points are the mean paired difference in R²; bars are 95% bootstrap CIs; grey = not significant after FDR (CI touches 0). Every contrast except colour early−V4 excludes zero.](figures/fig4_contrasts.png){#fig-stats width=85%}
...
cortex decodes color best (@tbl-color, @fig-color): concept beats early visual
