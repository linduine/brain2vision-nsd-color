...
- **Color → higher visual cortex.** The `concept` pool (all higher visual cortex)
  decodes the image's color distribution best (R² = 0.044 ± 0.004), because color
  is bound to object and scene identity.
- **Luminance → early visual cortex.** `early_v1v3` is the only region that
  decodes brightness as well as it decodes color (luminance R² = 0.029 ± 0.003),
  and it owns the dark/bright extremes.
...
![Functional dissociation between color and luminance decoding, by ROI (all 8 subjects, each ROI matched to 687 voxels; error bars are across-subject SEM). Higher visual cortex is strongly color-biased; early visual cortex is balanced; V4 is color-biased but weak on both raw measures.](figures/fig3_dissociation.png){#fig-dissociation width=70%}
...
- **H3 — supported.** Early visual cortex decodes brightness best and owns the
  dark bins (@sec-luminance), confirming a luminance signature.
...
the worst region — an artifact, not biology. Tuning `alpha` per fit with
`RidgeCV` (leave-one-out CV over a grid) fixes this; the selected `alpha` scales
with ROI size (V4 ≈ 5.3k, early ≈ 15k, concept ≈ 43k), and all overfitting
vanishes.
...
subsampled to the smallest ROI's size (V4's **687 voxels**), decoded, and
averaged over 10 random draws. Only after this does a ranking mean anything.
...
cohort (0.044 vs 0.032, well beyond the combined SEM).
...
: Color decoding, all 8 subjects, matched to 687 voxels. Chance top-1 ≈ 0.091. {#tbl-color}

![Per-color decoding R² by ROI (8 subjects, matched to 687 voxels; error bars = across-subject SEM). Higher visual cortex leads on chromatic, object-bound colors (brown, blue, green, orange); early visual cortex is the standout for black.](figures/fig1_color_by_roi.png){#fig-color}

The per-color structure is the interesting part. Higher visual cortex leads on
...
now reliable (0.029 vs 0.016, ~3.6× the combined SEM — it was only marginal at
n = 4).
...
: Luminance decoding, all 8 subjects, matched to 687 voxels. {#tbl-luminance}

![Per-brightness-bin decoding R² by ROI (L0 = dark → L10 = bright). The luminance signal is concentrated in the dark bins, where early visual dominates (L1 = 0.099, the single strongest value anywhere) and V4 is near zero or negative (L0 = −0.006).](figures/fig2_luminance_by_roi.png){#fig-luminance}
...
- **Probe perceptual colour, not just pixel colour.** Targets that capture colour
  constancy or colour *categories* (rather than raw-pixel histograms) would
  actually test V4's putative specialty, which the present targets do not.
