...
ROI differences are tested across subjects — the
[`stats`](https://github.com/linduine/brain2vision-nsd-color/blob/main/brain2vision/stats.py)
module — not read off the SEM bars. Each contrast uses an exact paired
**sign-flip permutation test** (all 2⁸ sign combinations), a 95% **bootstrap
confidence interval**, and **Benjamini-Hochberg FDR** correction across the family
of contrasts. With n = 8 the permutation *p* has a floor of 2/2⁸ ≈ 0.008 — so
*p* = 0.008 means the observed difference is more extreme than all other sign
combinations, i.e. as strong as eight subjects can show, not a marginal result.

All eight contrasts are in @tbl-stats and @fig-stats. Every ROI comparison is
significant after FDR correction **except** early visual vs V4 on colour, which is
a true tie — and the *bias* contrast (concept − early on colour-minus-luminance,
the double-dissociation interaction) is the largest and most reliable of all.
